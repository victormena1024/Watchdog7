...
//
// Two header files into one:
// SDL_net and WatchdogClient3.h
//
// Include SDL_net, which includes SDL.h for us
...
#ifndef WATCHDOGCLIENT3_H
#define WATCHDOGCLIENT3_H

#include <iostream>
#include <string>
#include <sstream>
#include <cstdarg>	// for variable argument list
using std::string;
//using std::cout;
//using std::cerr;
//using std::endl;
...
private:
	...
	int		begin = 0;					// offset into transmission buffer
	char	sdestinationLine[4096 + 7]; // transmission buffer: 4096 bytes
	...
	// ##############################################
	void wd_queueCommand(char *baffer);// internal use only
	// ##############################################

	// function to send data to server
	void sendCommand(char *stringCommand);
...
//	static const string       QUIT_SIGNAL;
	...
	bool failedCommunication = true;

	ClientSocket3(std::string theServerAddress, unsigned int theServerPort, unsigned int theBufferSize);
	~ClientSocket3();
...
//	bool getShutdownStatus();
	...
	// ##############################################
	// ##############################################
	void wd_sendInteger(int);						// A
	void wd_sendIntegerX(int);						// B
	void wd_plotRGB(int red, int green, int blue);	// C
	void wd_sendText(char *);						// D
	void wd_sendTextLines(char *);					// E
	void wd_plotXY(int x, int y, int color);		// F
	void wd_plotBinary(int);						// G
	void wd_BEGIN();								// 
	void wd_flush(void);							// send all queued commands
	void wd_END();									//
	void wd_log(const char *format, ...);			// send a printf formatted buffer through wd_sendTextLines(...)
													// and send immediately the result
	// ##############################################
	// ##############################################
};
