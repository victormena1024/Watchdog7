	...
	if (activeSockets != 0)
	{
		...
		if (gotMessage != 0)
		{
			...
			else // If we've received a 0 byte message from the server then we've lost the connection!
			{
				...
				if (debug) { cout << "Lost connection to the server!" << endl; }
...
void ClientSocket::getUserInput()
{
	...
	if (status != 0)
	{
		...
		else // Otherwise (if the user pressed enter) then send the message
		{
			...
			strcpy_s( pBuffer, 511, userInput.c_str() );
	...
	//			char fixbuffer[] = "E10909:E10A0A:E10B0B:E10C0C:E10D0D:E10E0E:E10F0F:E11010:\
	//			E38080:E38181:E38282:E88888:E39999:E3AAAA:E3BBBB:E3CCCC:\0\0\0";
	//			SDLNet_TCP_Send(clientSocket, (void *)fixbuffer, strlen(fixbuffer)+1 );
	//	exit(0);
...
// Configuration Properties->General->Item Type and change its 
// value to C / C++ compiler instead of C / C++ header.
