...
//const string ServerSocket::SHUTDOWN_SIGNAL = "/shut";
//const string ServerSocket::SHUTDOWN_SIGNAL = "/s";	// ********************
// ServerSocket::SHUTDOWN_SIGNAL

#include "Commands.h"
Commands *commandLayer = new Commands();

/*
#ifndef VICLOG_H
#define VICLOG_H
void vicLog(const char *format, ...);
#endif
void vicLog(const char *format, ...)
{
	char buffer[512 + 77];
	...
	count = _vsnprintf(buffer, 512 - 1, format, parms);
...
void vicLog(const char *format, ...)
{
	...
	int count = 0;
	va_start(parms, format);
	count = _vsnprintf_s(buffer, 512 - 1, format, parms);
// TODO, write string to graphical output
    _vsnprintf_s(buffer, 511,511, format, parms);
	//	ima->
	//MessageBox((HWND) nullptr, (LPCTSTR)buffer, (LPCTSTR) "=== main ===", MB_OK);
	va_end(parms);
}
...
void ServerSocket::checkForConnections()
{
	...
	if (serverSocketActivity != 0)
	{
		...
		if (clientCount < maxClients)
		{
			...
			strcpy_s( pBuffer, 512 ,SERVER_NOT_FULL.c_str() ); // 512 is a guessed value
		...
		else // If we don't have room for new clients...
		{
			...
			strcpy_s(pBuffer, 512, SERVER_FULL.c_str()); // 512 is a guessed value
			int msgLength = strlen(pBuffer) + 1;
...
// returns -1 on end of seekable characters
//
int seekToNextCharacter(char *str, char character);
int seekToNextCharacter(char *str, char character){
	int size = strlen( str );
...
#define TRANSMISSIONBUFFER_SIZE 4096
...
//	debug = true;
	if (debug) {
		std::cout << "Received: >>>> '" << bufferContents << "' from client number: " << clientNumber << endl;
	}

	char incomingString[TRANSMISSIONBUFFER_SIZE+1];

	char cc;
	unsigned int i;
	for (i = 0; i < bufferContents.size(); i++){
		cc = (char) bufferContents[i];
		incomingString[ i ] = cc;
		if (cc == 0) break;
	}
	incomingString[i] = 0; // last
	incomingString[i+1] = 0; // last

	// ******************************************************** //	
	char commandString[TRANSMISSIONBUFFER_SIZE];	// single commands
	int  tokens = 0;
	for (i = 0; i < TRANSMISSIONBUFFER_SIZE - 1; i++)
	{
		cc = (char)incomingString[i];
		if (cc == ':')	tokens++;
		if (cc == 0){ break; }
	} // 
	...
	{
		for (int u = 0; u < TRANSMISSIONBUFFER_SIZE - 2; u++)
		{
			char c = incomingString[u + (index2 + 1)];
			if (c == ':') c = 0;
			commandString[u] = c;
			if (c == '\0') break;
		}
		index1 = index2 + 1;
		index2 = seekToNextCharacter(&incomingString[index1], ':') + index1;
		if (index2 == -1) break;
		commandLayer->executeCommand((commandString[0] - 1), &commandString[1]);
	}
	commandLayer->tickExecuteDisplay();
...
//	cout << "       string:" << bufferContents.c_str << "\n";
//	cout << "length string:" << bufferContents.length << "\n";
/*
	...
	for (unsigned int loop = 0; loop < maxClients; loop++)
	{
		...
		{
			if (debug) {
				std::cout << "Retransmitting: " << bufferContents << " (" << msgLength << " bytes) to client " << loop << endl;
			}
